...
void LeggedStateEstimator::update(const Eigen::Vector3d& imu_gyro_raw, 
  ...
  std::vector<std::pair<int, bool>> contact_state_vector;
  for(int i = 0; i < 4; i++) {
    contact_state_vector.push_back(std::make_pair(i, contactFlags[i]));
  }
  std::vector<double> contactForceCovariance(4, 0.001);
  for(int i  = 0; i < 4; ++i) {
    if(contactFlags[i]) {
      contactForceCovariance[i] = 1.0;
    }
    else {
      contactForceCovariance[i] = 100.0;
    }
  }
  inekf_.setContacts(contact_state_vector);
  for (int i=0; i<robot_model_.numContacts(); ++i) {
    leg_kinematics_[i].setContactPosition(
        robot_model_.getContactPosition(i)-robot_model_.getBasePosition());
    const double contact_force_cov = contactForceCovariance[i];

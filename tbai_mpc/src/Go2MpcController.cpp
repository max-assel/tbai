...
Go2MpcController::Go2MpcController(const std::shared_ptr<tbai::core::StateSubscriber> &stateSubscriberPtr)
...
void Go2MpcController::spinOnceReferenceThread() {
    referenceThreadCallbackQueue_.callAvailable(ros::WallDuration(0.0));
}

tbai_msgs::JointCommandArray Go2MpcController::getCommandMessage(scalar_t currentTime, scalar_t dt) {
    mrt_.spinMRT();
    mrt_.updatePolicy();
...
void Go2MpcController::referenceThread() {
    ...
    while (ros::ok() && !stopReferenceThread_) {
        spinOnceReferenceThread();
        ROS_INFO_STREAM_THROTTLE(5.0, "[Go2MpcController] Publishing reference");
        referenceTrajectoryGeneratorPtr_->publishReferenceTrajectory();
        rate.sleep();
    }
}

bool Go2MpcController::checkStability() const {
...
void Go2MpcController::visualize() {
...
void Go2MpcController::changeController(const std::string &controllerType, scalar_t currentTime) {
...
void Go2MpcController::startReferenceThread() {
    // Start reference thread
    if (referenceThread_.joinable()) {
        referenceThread_.join();
    }
    stopReferenceThread_ = false;
    referenceThread_ = std::thread(&Go2MpcController::referenceThread, this);
}

void Go2MpcController::stopReferenceThread() {
    stopReferenceThread_ = true;
}

bool Go2MpcController::isSupported(const std::string &controllerType) {
    return controllerType == "WBC";
}

void Go2MpcController::resetMpc() {
...
void Go2MpcController::setObservation() {
    mrt_.setCurrentObservation(generateSystemObservation());
    timeSinceLastMpcUpdate_ = 0.0;
}

ocs2::SystemObservation Go2MpcController::generateSystemObservation() const {
